...
def shock(finalhhframe,sh_people_affected,losses_poor,losses_rich):
	...
	if sh_people_affected<=0:
		...
		isbelowline                            = (finalhhframe21['Y']<=10*365)#float(th))
		#apply income shocks
...
def get_model_cc_parameters(food_cursor, temp_impact, paramshstunt, malaria_cursor, diarrhea_cursor, disasters_cursor, ssp, wbreg, 
			    price_increase_reg, malaria_bounds, diarrhea_bounds, finalhhframe, shares_stunting, malaria_diff, 
			    countrycode, diarrhea_shares, diarrhea_reg, climate_param_bounds, climate_param_thresholds,fa,
			    vulnerabilities,voice):
	'''
	Loads all climate change parameters from data sources.
	'''
		
	#until we find something better we don't change the frequency of the disease
	...
	try: 
		...
		losses_poor,losses_rich = 0,0
		fa_poor,fa_nonpoor = 0,0
	...
	try: lostdays_mal,eventcost_mal = get_disease_impact(malaria_cursor,malaria_bounds)
	except: lostdays_mal,eventcost_mal = 0,0

	try: lostdays_dia,eventcost_dia = get_disease_impact(diarrhea_cursor,diarrhea_bounds)
	except: lostdays_dia,eventcost_dia = 0,0
	shares_outside             = get_shares_outside(finalhhframe)
	
	try: sh_people_stunt            = get_shares_stunting(wbreg,paramshstunt,shares_stunting,ssp)
	except: sh_people_stunt  = 0

	try: malaria_share              = get_malaria_share(countrycode,wbreg,malaria_diff)
	except: malaria_share = 0
	...
	try: diarrhea_share             = diarrhea_increase*diarrhea_reg.who.replace(diarrhea_shares.ratio).fillna(0)[reverse_correct_countrycode(countrycode)]/diarrhea_yr_occ
	except: diarrhea_share = 0
	...
	except: flood_share_poor, flood_share_nonpoor, drought_share, wind_share, surge_share  = 0,0,0,0,0
